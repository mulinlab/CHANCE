...
my $fo1 = "../output/03_filter_snv_in_pathogenicity_icgc.txt.gz";
    ...
    unless(/start/){
        ...
        if($hgvsg =~/chr/){
            ...
            if (exists $hash1{$k}){
                my $id= $hash1{$k};
                # print STDERR "$k\n";
                print $O1 "$hgvsg\t$id\t$submitted_donor_id\t$gene\t$Variant_Classification\t$oncotree_detail_ID\t$oncotree_main_tissue_ID\n";
            }
        }
        else{
            $hgvsg ="chr${hgvsg}";
